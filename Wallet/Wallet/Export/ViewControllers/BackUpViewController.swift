...
protocol BackupViewControllerDelegate: class {
    func didPressBackup(account: TrustKeystore.Account, in viewController: BackupViewController)
}

class BackupViewController: UIViewController {

    let account: TrustKeystore.Account
    weak var delegate: BackupViewControllerDelegate?
    let viewModel = BackupViewModel()

    init(account: TrustKeystore.Account) {
        ...
        controlMoneyLabel.textColor = UIColor.disabledTextColor
        controlMoneyLabel.font = UIFont.systemFont(ofSize: 14, weight: .medium)
        ...
        neverStoredLabel.textColor = UIColor.disabledTextColor
        neverStoredLabel.font = UIFont.systemFont(ofSize: 14, weight: .medium)
        ...
        backupButton.setBackgroundColor(UIColor.mainNormalColor, forState: .normal)
        backupButton.setTitleColor(UIColor.white, for: .normal)
        backupButton.pborderColor = UIColor.mainNormalColor
        backupButton.pcornerRadius = 2
