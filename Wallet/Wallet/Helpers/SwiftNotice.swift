...
extension UIResponder {
    ...
    @discardableResult
    func pleaseWaitWithImages(_ imageNames: Array<UIImage>, timeInterval: Int, text: String = "") -> UIWindow{
        return SwiftNotice.wait(imageNames, timeInterval: timeInterval, text: text)
    }
...
class SwiftNotice: NSObject {
    ...
    @discardableResult
    static func wait(_ imageNames: Array<UIImage> = Array<UIImage>(), timeInterval: Int = 0, text: String = "") -> UIWindow {
        ...
        if imageNames.count > 0 {
            ...
            ai.snp.makeConstraints({ (make) in
                make.centerY.equalTo(mainView).offset(-5)
        ...
        label.textColor = UIColor.white
        label.text = text
        label.center = CGPoint(x: mainView.center.x, y: label.frame.minY)
        label.textAlignment = NSTextAlignment.center
    ...
    static func showNoticeWithText(_ type: NoticeType,text: String, autoClear: Bool, autoClearTime: Int) -> UIWindow {
        ...
        var stringSize: CGSize = text.stringSize(font: UIFont.systemFont(ofSize: 13), maxWidht: 260/320*UIScreen.main.bounds.width )
        stringSize.width += 2*padding
        
        let frame = CGRect(x: 0, y: 0, width: stringSize.width > 90 ? stringSize.width : 90, height: 74 + stringSize.height)
        let window = UIWindow()
        window.backgroundColor = UIColor.clear
        let mainView = UIView()
        ...
        checkmarkView.snp.makeConstraints { (make) in
            make.centerY.centerX.equalTo(mainView)
            make.centerY.equalTo(mainView).offset(-10)
        }
        
        let label = UILabel(frame: CGRect(x: padding, y: 60, width: stringSize.width - 2*padding, height: stringSize.height))
        label.font = UIFont.systemFont(ofSize: 13)
        label.numberOfLines = 0
        label.textColor = UIColor.white
        label.text = text
